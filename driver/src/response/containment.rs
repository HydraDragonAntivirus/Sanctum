...
use wdk_sys::{ntddk::{ZwOpenProcess, ZwTerminateProcess}, CLIENT_ID, NTSTATUS, OBJECT_ATTRIBUTES, OBJ_KERNEL_HANDLE, PROCESS_ALL_ACCESS};
...
fn terminate_process(
    ...
    let mut handle: *mut c_void = null_mut();
        ...
        let status = unsafe { ZwTerminateProcess(
            handle,
            1
        ) };
